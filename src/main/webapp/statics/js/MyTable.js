			...
			success: function(data){
				if(data.retcode=="0000"){
					...
					$.each(list, function(index, item) {
						...
						//自己增加
						$(createObj).find("#speciala").attr("href","/mobile/carUsage?id="+item.id);
						$(createObj).find("#peasantspeciala").attr("href","/mobile/peasantUsage?id="+item.id);
						$.each(columns[tableId], function(index1, item1) {
							...
							if(type=="INPUT"){
								...
								$(createObj).find("#"+mDataProp).attr("src","http://"+host+item[mDataProp]);
							}
							else{
								...
								if(mDataProp=="vipTime"){
									var temp=item[mDataProp].substring(0,10).replace(/-/g,"/");
									item[mDataProp]=temp;
								}
				...
				if(data.retcode=="0000"){
					...
					if(div==null){
						...
						$.each(columns[tableId], function(index, item) {
							...
							if(type=="INPUT"){
								if(mDataProp=="vipTime"){
									var value=value.substring(0,10).replace(/-/g,"/");
								}
